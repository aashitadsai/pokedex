...
export class PokeapiService {
  ...
  private pokeapi_list:string = "https://pokeapi.co/api/v2/pokemon/?limit=20&amp;offset=";
  ...
  getPokeList():Promise<any> {
    return this.http.get(this.pokeapi_list+this.count)
      .toPromise()
      .then(data=>{
  ...
  getAbilityDetail(ability_url:string):Promise<abilities> {
    ...
    return this.http.get(ability_url)
    .toPromise()
    .then(data => {
  ...
  getPokeDetail(pokemon_url:string):Promise<pokemon_detail> {
    ...
    .then(data => {
      ...
      data['abilities'].map(element => {
        this.getAbilityDetail(element['ability']['url'])
        .then(a=>{
